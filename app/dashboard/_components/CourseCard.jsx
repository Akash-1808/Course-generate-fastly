...
function CourseCard({course,refreshData,displayUser=false}) {
  ...
  return (
    <div className='shadow-sm rounded-lg flex flex-col gap-1 border p-2
     cursor-pointer mt-4 '>
      <Link href={'/course/'+course?.courseId}>
        <Image src={course?.courseBanner} width={300} height={200} alt='loading'
        className='w-fullh h-[200px] object-cover rounded-lg'/>
        </Link>
        <div className='p-2'>
            ...
            <div className='flex items-center justify-between'>
              <h2 className='flex gap-2 items-center p-1 bg-purple-50 text-primary text-sm rounded-sm'><HiOutlineBookOpen/> {course?.courseOutput?.numberOfChapters} Chapters</h2>
                <h2 className='test-sm bg-purple-50 text-primary p-1 rounded-sm'>{course?.level}</h2>
            </div>
           {displayUser && <div className='flex gap-2 items-center mt-2'>
              <Image src={course?.userProfileImage} width={19} height={19} className='rounded-full'/>
